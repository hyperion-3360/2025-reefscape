...
public class DriveToPose implements PoseToSpeed {
  ...
  private StructPublisher<Pose2d> autoAmpPosePublisher =
      NetworkTableInstance.getDefault()
          .getStructTopic("Vision/autoAmpRobotPose", Pose2d.struct)
          .publish();
  ...
  public DriveToPose(
      double kPtranslation, double kProtation, double translationMarge, double rotationMarge) {
    ...
    autoAmpPosePublisher =
        NetworkTableInstance.getDefault()
            .getStructTopic("Vision/autoAmpRobotPose", Pose2d.struct)
            .publish();

    pidConstants =
        new TrapezoidProfile.Constraints(PID_PROFILE_MAX_VELOCITY, PID_PROFILE_MAX_ACCELERATION);
    pidDriveToPosX = new ProfiledPIDController(kPtranslation, 0, 0, pidConstants, kHIDLooperDt);
    pidDriveToPosY = new ProfiledPIDController(kPtranslation, 0, 0, pidConstants, kHIDLooperDt);
  }
  ...
  public ChassisSpeeds getTargetSpeeds(
      Pose2d currentPose, PathPlannerTrajectoryState targetTrajectoryState) {
    return null;
  }
  ...
  @Override
  public ChassisSpeeds getTargetSpeeds(
      Pose2d currentPose, Pose2d targetPose) { // ChassisSpeeds currentSpeeds) {
    // reset of the pid
