...
public class Vision {
  ...
  public Vision() {
    ...
    photonEstimator = new PhotonPoseEstimator(tagLayout, PoseStrategy.MULTI_TAG_PNP_ON_COPROCESSOR,robotToCam);
  ...
  public Optional<EstimatedRobotPose> getEstimatedGlobalPose() {
    ...
    for (var change : camera.getAllUnreadResults()) {
      ...
      if (Robot.isSimulation()) {
        visionEst.ifPresentOrElse((est -> getSimDebugField().getObject("VisionEstimation").setPose(est.estimatedPose.toPose2d())), () -> {getSimDebugField().getObject("VisionEstimation").setPoses();});
      }
    }
    return visionEst;
  }
